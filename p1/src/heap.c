...
Error Heapify(Heap* heap, int index) {
    ...
    // Variables which will hold the max key of the children
    ...
    if (isSome(&right)) {
        ...
        // And assign the max between them to the maxChildKey and maxChildIdx respectively
        if (leftKey > rightKey) {
            maxChildKey = leftKey;
        ...
        // Otherwise if the left exists then it is the only child so it must be max
    ...
    // If the max child key is larger than the curent key then we swap the elements
...
Error IncreaseKey(Heap* heap, int index, int value, int flag) {
    ...
    // Make sure the new key value is actually larger than the current
    if (value < currKey) {
    ...
    // until the parent key is greater than the current key or until we reach
    // the top of the "tree"
...
Error Insert(Heap* heap, int k, int flag) {
    ...
    // Use IncreaseKey operation on the element with the proper key value
...
Error DeleteMax(Heap* heap, int flag) {
    ...
    // Decrease heap size to excluded swapped max
    heap->size--;

    // Heapify on root to preserve max heap property
    Heapify(heap, 0);
